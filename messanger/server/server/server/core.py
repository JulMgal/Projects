...
import binascii
import os
from server.server.common.metaclasses import ServerMaker
from server.server.common.descryptors import Port
from server.server.common.variables import *
from server.server.common.utils import send_message, get_message
from server.server.common.decos import login_required
...
class MessageProcessor(threading.Thread):
    ...
    def autorize_user(self, message, sock):
        if message[USER][ACCOUNT_NAME] in self.names.keys():
            ...
            hash = hmac.new(
                self.database.get_hash(message[USER][ACCOUNT_NAME]), random_str)
            digest = hash.digest()
            try:
